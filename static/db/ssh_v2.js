...
function saveHotKey(sql) {
    ...
    for (let i = 0; i < sql.length; i++) {
        ...
        if (!hotCount[k]) // 新词
            hotCount[k] = [0, 0]; // [出现次数, 最后使用时间]
        hotCount[k] = [hotCount[k][0] + 1, now]; // 统计
        k = '';
    }
    localStorage.setItem('db_hot_key_count', JSON.stringify(hotKeys));
    saveHotTable(sql);
}
...
function showHotKey(input, left, top) {
    ...
    let hotKey = JSON.parse(localStorage.getItem('db_hot_key_count'));
...
function cacheClear() {
    let hots = JSON.parse(localStorage["db_hot_key"]);
    // {key: [统计， 下标, 最后使用时间]}
    let hotCount = JSON.parse(localStorage["db_hot_key_count"]);
    let hotLen = hots.length;
    for (let i = hots.length-1; i >= 0; i--) {
        if (hotCount[hots[i]][0] > 4) {
            break;
        }
        hots.pop();
        delete hotCount[hots[i]];
    }
    for (let i = hots.length-1; i >= 0; i--) {
        if (hotCount[hots[i]][0] > 10) break;
        let prefix = hotCount[hots[i]][0] > 1 ? '>>>' : '---';
        console.log(`${prefix} [${hotCount[hots[i]][0]}] ${hots[i]}`);
    }
    console.log(`${hotLen} / ${hots.length}`);
    // localStorage.setItem('db_hot_key', JSON.stringify(hots));
